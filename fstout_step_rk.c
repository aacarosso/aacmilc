...
void fstout_step_rk(su3_matrix *S[4], anti_hermitmat *A[4], double eps, int last){
  ...
  // Stores W matrices for last step.
  if ( last == 1 ){
    for (dir = 0; dir < 4; dir++){
      FORALLSITES(i, s){
        link = &(s->link[dir]);
        su3mat_copy(link, &(s->W0[dir]));
        //su3mat_copy(link,&(W0[dir][i]));
      }
    }
  }
  staple(S);
  for (dir = 0; dir < 4; dir++){
    update_flow(dir, A[dir], S[dir], 17 * eps / 36.0, -9 / 17.0);
    if ( last == 1 ){
      FORALLSITES(i, s){
        link = &(s->link[dir]);
        su3mat_copy(link, &(s->W1[dir]));
      }
    }
  }
  staple(S);
  for (dir = 0; dir < 4; dir++){
    update_flow(dir, A[dir], S[dir], -8 * eps / 9.0, 1);
    if ( last == 1 ){
        FORALLSITES(i, s){
        link = &(s->link[dir]);
        su3mat_copy(link, &(s->W2[dir]));
      }
    }
  }
  staple(S);
  for (dir = 0; dir < 4; dir++){
    update_flow(dir, A[dir], S[dir], 3 * eps / 4.0, -1);
    if ( last == 1 ){
      FORALLSITES(i, s){
        link = &(s->link[dir]);
        su3mat_copy(link, &(s->W3[dir]));
      }
    }
  }
}
