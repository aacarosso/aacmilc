...
int fCG_wrapper(field_offset chi, field_offset W, su3_vector **props, Real m, int parity) {
  register int i, it;
  register site* s;

  it = fks_congrad(chi, W, F_OFFSET(psi), m, parity);
  FORALLSITES(i, s)
...
int fks_congrad(field_offset chi, field_offset W, field_offset psi, Real m, int parity) {
  register int i;
  register site *s;
...
start:
  ...
  rsq = 0;
  source_norm = 0;
  fdslash(psi, W, F_OFFSET(mp), l_otherparity);
  fdslash(F_OFFSET(mp), W, F_OFFSET(mp), l_parity);
  ...
  do { // while (iteration % niter != 0);
    ...
    if (special_started == 0) {
      fdslash_special(F_OFFSET(p), F_OFFSET(mp), l_otherparity, tags2, 1);
      fdslash_special(F_OFFSET(mp), F_OFFSET(mp), l_parity, tags1, 1);
      special_started = 1;
    }
    else {
      fdslash_special(F_OFFSET(p), F_OFFSET(mp), l_otherparity, tags2, 0);
      fdslash_special(F_OFFSET(mp), F_OFFSET(mp), l_parity, tags1, 0);
    }
...
void fdslash_wrapper(su3_vector *src, field_offset W, su3_vector *dest, int parity) {
  ...
  fdslash(F_OFFSET(chi), W, F_OFFSET(psi), parity);
  FORALLSITES(i, s)
...
void fdslash(field_offset chi, field_offset W, field_offset psi, int parity) {
  ...
  FORSOMEPARITYDOMAIN(i, s, otherparity) {
    if (i < loopend - FETCH_UP)
      prefetch_4MV4V((su3_matrix *)F_PT((s + FETCH_UP),W_off[XUP])/*
                     &((s + FETCH_UP)->link[XUP])*/,
                     (su3_vector *)F_PT((s + FETCH_UP), chi),
                     (s+FETCH_UP)->tempvec);

    mult_adj_su3_mat_vec_4dir((su3_matrix *)F_PT(s,W)/*s->link*/, (su3_vector *)F_PT(s, chi),
                              s->tempvec);
  } END_LOOP
  ...
  FORSOMEPARITY(i, s, parity) {
    if (i < loopend - FETCH_UP) {
      ...
      prefetch_4MVVVV((su3_matrix *)F_PT((s + FETCH_UP),W_off[XUP])/*&((s +
                      FETCH_UP)->link[XUP])*/,
    ...
    mult_su3_mat_vec_sum_4dir((su3_matrix *)F_PT(s,W)/*s->link*/,
                              (su3_vector *)gen_pt[XUP][i],
...
void fdslash_special(field_offset chi, field_offset psi, int parity,
  ...
  FORSOMEPARITYDOMAIN(i, s, otherparity) {
    if (i < loopend-FETCH_UP)
      prefetch_4MV4V(&((s + FETCH_UP)->W0[XUP]),
                     (su3_vector *)F_PT((s + FETCH_UP), chi),
                     (s + FETCH_UP)->tempvec);

    mult_adj_su3_mat_vec_4dir(s->W0, (su3_vector *)F_PT(s, chi),
                              s->tempvec);
  } END_LOOP
  ...
  FORSOMEPARITY(i, s, parity) {
    if (i < loopend - FETCH_UP)
      prefetch_4MVVVV(&((s + FETCH_UP)->W0[XUP]),
                      (su3_vector *)gen_pt[XUP][i + FETCH_UP],
                      (su3_vector *)gen_pt[YUP][i + FETCH_UP],
                      (su3_vector *)gen_pt[ZUP][i + FETCH_UP],
                      (su3_vector *)gen_pt[TUP][i + FETCH_UP]);

    mult_su3_mat_vec_sum_4dir(s->W0, (su3_vector *)gen_pt[XUP][i],
                              (su3_vector *)gen_pt[YUP][i],
                              (su3_vector *)gen_pt[ZUP][i],
                              (su3_vector *)gen_pt[TUP][i],
                              (su3_vector *)F_PT(s,psi));
