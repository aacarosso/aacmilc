...
// modified to accept whatever gauge field config offset is given: W[4]
...
complex fploop(field_offset W, int dir) {
  ...
  msg_tag *tag;
  complex sum, plp;
  int d[4], N = nt, W_off[4];

  sum = cmplx(0, 0);
  for (i = XUP; i <= TUP; i++){
    d[i] = 0;
    W_off[i] = W + i*sizeof(su3_matrix);
  }
  ...
  tag = start_gather_site(W_off[dir], sizeof(su3_matrix),
                          dir, EVEN, gen_pt[0]);

  wait_gather(tag);
  FOREVENSITES(i, s)
    mult_su3_nn((su3_matrix *)F_PT(s,W_off[dir]), (su3_matrix *)gen_pt[0][i],
