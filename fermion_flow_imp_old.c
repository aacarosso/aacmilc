...
void fermion_flow_imp(double eps_max) {
  ...
  double t_epsmax=0, *eps, dt, flowtime=0, sum_eps, ttime=0, tvar, tvar1;
  Real t, cut = 1e-7;
  complex dot1;
  ...
  for (istep = 0; t > cut; istep++){
    ...
    if (t - t_epsmax > cut){
      ...
      //wflow_imp(eps[j],F_OFFSET(link1)+4*l*sizeof(su3_matrix), t_epsmax+l*dt, tmax-sum_eps, eps_max);
      wflow_imp(eps[j],F_OFFSET(link0)+4*(l+1)*sizeof(su3_matrix), t_epsmax+l*dt, t-epsilon, eps_max);
    }
    else {
      //node0_printf("IMPROV2: ti = 0 tf = %g\n",tmax-sum_eps);
      //wflow_imp(epsilon,F_OFFSET(link0), 0, tmax-sum_eps, eps_max);
